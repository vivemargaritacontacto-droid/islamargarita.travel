import type { Config } from "tailwindcss";

const config: Config = {
  content: [
    "./pages/**/*.{js,ts,jsx,tsx,mdx}",
    "./components/**/*.{js,ts,jsx,tsx,mdx}",
    "./app/**/*.{js,ts,jsx,tsx,mdx}",
    "./src/**/*.{js,ts,jsx,tsx,mdx}",
  ],
  theme: {
    extend: {
      colors: {
        primary: "#1B396A",
        gold: "#C9A84C",
        "gold-hover": "#b8943d",
        "navy-dark": "#0F2347",
        "navy-light": "#24508A",
        "bg-light": "#F6F7F8",
        // Tropical palette (playa-el-angel landing)
        turquoise: "#0891B2",
        "turquoise-dark": "#0E7490",
        "turquoise-light": "#CFFAFE",
        coral: "#F97316",
        "coral-dark": "#EA580C",
        sunshine: "#FACC15",
        sand: "#FEF3C7",
        cream: "#FEFCE8",
        ink: "#0F172A",
      },
      fontFamily: {
        sans: ["var(--font-poppins)", "Poppins", "sans-serif"],
        poppins: ["var(--font-poppins)", "Poppins", "sans-serif"],
        pacifico: ["var(--font-pacifico)", "Pacifico", "cursive"],
      },
      boxShadow: {
        gold: "0 4px 24px rgba(201,168,76,0.25)",
        "gold-lg": "0 8px 40px rgba(201,168,76,0.35)",
        navy: "0 4px 24px rgba(15,35,71,0.20)",
        "navy-xl": "0 20px 60px rgba(15,35,71,0.30)",
        card: "0 10px 25px -5px rgba(27,57,106,0.10), 0 8px 10px -6px rgba(27,57,106,0.08)",
        "tropical-sm": "0 2px 8px rgba(8, 145, 178, 0.08)",
        "tropical-md": "0 8px 30px rgba(8, 145, 178, 0.12)",
        "tropical-lg": "0 20px 60px rgba(8, 145, 178, 0.18)",
        coral: "0 8px 25px rgba(249, 115, 22, 0.45)",
        "coral-lg": "0 12px 35px rgba(249, 115, 22, 0.55)",
      },
      backgroundImage: {
        "hero-gradient": "linear-gradient(to bottom right, #0F2347, #24508A)",
      },
      animation: {
        "spin-slow": "spin 8s linear infinite",
        float: "float 4s ease-in-out infinite",
      },
      keyframes: {
        float: {
          "0%, 100%": { transform: "translateY(0px)" },
          "50%": { transform: "translateY(-6px)" },
        },
      },
    },
  },
  plugins: [],
};
export default config;
